<template>
    <div class="">
        ...
        <el-link type="primary" href="http://localhost:8080/ExcelDownloads"><h2>点此导出Excel报表</h2></el-link>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            this.$axios({      
                    method: 'post',
                    url: 'http://localhost:8080/selForWait',
                    params:{
        ...
        findCount(){
            this.$axios({        
                    method: 'post',
                    url: 'http://localhost:8080/findappTotal',
                    }).then((result) => {
        ...
        handlePast(index, row) { 
            this.$confirm('确定要下发吗？', '提示', {
                ...
                .then(() => {
                    this.$axios({        
                    method: 'post',
                    url: 'http://localhost:8080/updSuccess',
                    params:{
                        id:row.id,
                        issuccess:this.ispast.past
                    },
                    }).then((result) => {
                        if(result.data==200);
        ...
        exportTable(){
             this.$axios({ 
                    method: 'post',
                    url: 'http://localhost:8080/ExcelDownloads',
                    }).then((result) => {
